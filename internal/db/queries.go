...
func (s *Store) CreateJob(j *Job) error {
	res, err := s.db.Exec(`INSERT INTO jobs
		(deployment_id, agent, name, runtime, issue_number, issue_title, issue_body,
		 owner, repo, status, dependencies, stages_json)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		j.DeploymentID, j.Agent, j.Name, j.Runtime, j.IssueNumber, j.IssueTitle, j.IssueBody,
...
func (s *Store) UpsertSchedule(js *JobSchedule) error {
	_, err := s.db.Exec(`INSERT OR REPLACE INTO job_schedules
		(name, deployment_id, cron_expr, trigger_expr, agent, runtime, description, budget, max_turns, enabled, next_run_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		js.Name, js.DeploymentID, js.CronExpr, js.TriggerExpr,
		js.Agent, js.Runtime, js.Description, js.Budget, js.MaxTurns, js.Enabled, js.NextRunAt)
	return err
}
