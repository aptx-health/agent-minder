...
// SetTaskOverrides sets specific resource overrides on a task.
func (s *Supervisor) SetTaskOverrides(taskID int64, maxTurns *int, maxBudget *float64) error {
	return s.store.UpdateAutopilotTaskOverrides(taskID, maxTurns, maxBudget)
}

// ClearTaskOverrides removes all per-task resource overrides, reverting to project defaults.
func (s *Supervisor) ClearTaskOverrides(taskID int64) error {
	return s.store.UpdateAutopilotTaskOverrides(taskID, nil, nil)
}
