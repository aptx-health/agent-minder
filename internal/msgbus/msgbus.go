...
// OpenContext is like Open but respects context cancellation, preventing
// indefinite hangs when the database is unreachable (e.g., after laptop sleep).
func OpenContext(ctx context.Context, dbPath string) (*Client, error) {
	db, err := sqliteutil.OpenWithRecoveryContext(ctx, dbPath, dbPath+"?mode=ro&_journal_mode=WAL&_pragma=busy_timeout(5000)")
	if err != nil {
...
func NewPublisher(dbPath string) (*Publisher, error) {
	return NewPublisherContext(context.Background(), dbPath)
}

// NewPublisherContext is like NewPublisher but respects context cancellation.
func NewPublisherContext(ctx context.Context, dbPath string) (*Publisher, error) {
	db, err := sqliteutil.OpenWithRecoveryContext(ctx, dbPath, dbPath+"?_journal_mode=WAL&_pragma=busy_timeout(5000)")
	if err != nil {
