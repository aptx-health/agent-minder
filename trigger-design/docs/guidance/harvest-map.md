...
Investigation takeaways live in [[../harvest/index]] — one doc per row below, plus
[[../harvest/fable-expedition-crosswalk]] for the Fable expedition findings that
audited (and in the event log's case, designed) this hardening.

## Lift close to verbatim (proven, well-bounded)

| From | Package / area | Why it is worth transplanting | Harvest notes |
| --- | --- | --- | --- |
| agent-minder | `internal/sqliteutil` | WAL recovery, stale -shm/-wal cleanup, `OpenWithRecovery`. Restart-safety is a v1 invariant. | [[sqliteutil-wal-recovery]] |
| agent-minder | `internal/agentutil` | Agent-log / stream-json parsing, failure-signal detection. | [[agentutil-log-parsing]] |
| agent-minder | event log (store-first publish) | Durable event log with commit-as-publish; the future TUI/GUI stream depends on it. | [[event-log-store-first]] |
| agent-minder | `agent_runs` table shape | Durable per-step/attempt run records; the resumability basis for workflows ([[0008-workflows-deterministic-steps]]). | [[agent-runs-table]] |
| agent-minder | script execution config | `script_command`/`script_timeout`/`script_env`/`script_work_dir` for deterministic `script` steps ([[0008-workflows-deterministic-steps]] rule 2). | [[script-execution-config]] |
| agent-minder | `internal/git` worktree helpers | Worktree add/list, `worktreeinclude`, `setup.sh` run. Only if jobs use worktrees. | [[git-worktree-helpers]] |
| agent-minder | `checkout` command + `internal/auth` | Summon a job's worktree locally and jump to the PR (keep in the TUI); OS-keyring token storage (feeds [[0006-secrets-and-agent-permissions]]). | [[checkout-and-auth]] |
| pr-triage | opencode runtime adapter | Proven-working; the fastest path to dogfooding. Copy first, migrate to ACP after. | [[opencode-runtime-adapter]] · [[pr-triage-runtime-seam]] |
| pr-triage | Claude Code runtime adapter | Second proven runtime. | [[claudecode-runtime-adapter]] · [[pr-triage-runtime-seam]] |
...
- **Scheduler tick** — the 30s refill/tick loop pattern is sound; reimplement against the
  new job store. → [[scheduler-tick-pattern]]
...
- **Daemon HTTP API + client** — pattern is proven; rebuild narrow and explicit per
  [[0004-daemon-interface-split]]. → [[daemon-api-pattern]]
