...
- **HTTP API server**: Embeddable HTTP API server (`--serve :7749`) in deploy daemon exposing `/status`, `/tasks`, `/tasks/:id`, `/tasks/:id/log` (with SSE streaming), `/dep-graph`, `/analysis`, `/analysis/poll`, `/metrics`, and `/stop` endpoints; API key auth via `--api-key` flag or `MINDER_API_KEY` env var; CORS headers; graceful shutdown on SIGTERM
